...
void writeMotors() { // [1000;2000] => [125;250]
    ...
    #if (NUMBER_MOTOR > 0) 
      #ifndef EXT_MOTOR_RANGE  // <- geovas edition (start)
        OCR1A = motor[0] << 2; //  pin 9 - Rear Right
      #else
        if (motor[0] == MINCOMMAND) OCR1A = 0x00; else
        OCR1A = ((motor[0] << 3) - 8000) + 128;
      #endif
    #endif
    #if (NUMBER_MOTOR > 1)
      #ifndef EXT_MOTOR_RANGE 
        OCR1B = motor[1]>>3; //  pin 10 - Front Right
      #else
        if (motor[1] == MINCOMMAND) OCR1B = 0x00; else
        OCR1B = ((motor[1] << 3) - 8000) + 128;
      #endif
    #endif                   // <- geovas edition (end)
    #if (NUMBER_MOTOR > 2)
      #ifndef EXT_MOTOR_RANGE
        OCR2A = motor[2]>>3; //  pin 11 - Rear Left
      #else
        if (motor[2] == MINCOMMAND) OCR2A = 0x00; else
        OCR2A = ((motor[2]>>2) - 250) + 2;
      #endif
    #endif
    ...
    #if (NUMBER_MOTOR > 3)
      #ifndef EXT_MOTOR_RANGE
        OCR2B = motor[3]>>3; //  pin 3 - Front Left
      #else
        if (motor[3] == MINCOMMAND) OCR2B = 0x00; else
        OCR2B = ((motor[3]>>2) - 250) + 2;
      #endif
    #endif
    #if (NUMBER_MOTOR > 4) //note: EXT_MOTOR_RANGE not possible here
...
void initOutput() {
  for(uint8_t i=0;i<NUMBER_MOTOR;i++)
    ...
    #endif
    #if (NUMBER_MOTOR > 2)
      TCCR2A |= _BV(COM2A1); /*TCCR2B |= _BV(CS21) | _BV(CS20);*/ // connect pin 11 to timer 2 channel A prescale32
                                                                 // fixed by geovas
    #endif
    #if (NUMBER_MOTOR > 3)
      TCCR2A |= _BV(COM2B1) ; // connect pin 3 to timer 2 channel B
