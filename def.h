...
#if defined(ESP)
  #define LEDPIN_PINMODE             pinMode (LED_BUILTIN, OUTPUT); //LED PIN level inverted
  #define LEDPIN_TOGGLE              digitalWrite(LED_BUILTIN, !digitalRead(LED_BUILTIN)); //PINB |= 1<<5;     //switch LEDPIN state (digital PIN 13)
  #define LEDPIN_OFF                 digitalWrite(LED_BUILTIN, !digitalRead(LOW)); //PORTB &= ~(1<<5);
  #define LEDPIN_ON                  digitalWrite(LED_BUILTIN, !digitalRead(HIGH)); //PORTB |= (1<<5);
...
#if defined(ITG3200) || defined(L3G4200D) || defined(MPU6050) || defined (MPU3050) // <- geovas edition
  #define GYRO 1
#else
  #define GYRO 0
#endif
